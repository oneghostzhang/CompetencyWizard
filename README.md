...
| 功能 | 說明 |
|------|------|
| 📝 **5W2H 結構化輸入** | 引導填寫 What / Why / Who / When / Where / How / How Much 七個面向，執行頻率支援多選勾選框 |
| 📋 **職能基準快速填入** | 表單頁可選擇職能基準範本，自動預填工作任務（數字編號）、職務說明、技能等欄位，達到 80–90% 填寫完成度 |
| 🔍 **語意向量檢索** | `BAAI/bge-base-zh-v1.5` + FAISS，從 900+ 職能基準中找出最相似標準 |
| 📊 **職能缺口分析** | 自動比對輸入與職能基準，輸出完整度分數（0–100%）與缺口說明 |
| ✅ **逐項確認精靈** | 分析完成後自動開啟 `StandardAdoptionWizard`，以勾選方式逐項確認工作任務、知識、技能；已偵測項目綠色標示，支援全選／全不選，可重複調整直到滿意 |
...
啟動 → 自動載入 Embedding 模型 + 建立 FAISS 索引
  ↓
填寫 5W2H 表單
  ├── 可點選「選擇範本 →」從職能基準快速預填（省略 80–90% 輸入）
  └── 執行頻率：勾選框多選（每日／每週／每月／每季／專案型／其他）
  ↓
點選「開始分析 →」
  ↓
【自動開啟】職能基準逐項確認精靈：
  ├── 📋 工作任務頁：依主責分組，勾選實際執行的任務（已偵測項目綠色預勾）
  ├── 📖 知識頁：勾選實際具備的知識項目
  ├── 🔧 技能頁：勾選實際具備的技能項目
  └── 確認採用 → 系統重新計算完整度與缺口
  ↓
...
- `StandardSelectorDialog`：從職能基準資料庫選擇範本，自動預填 5W2H 表單
- `StandardAdoptionWizard`：分析完成後逐項確認精靈；三分頁（任務／知識／技能），已偵測項目綠色預勾，支援全選／全不選
